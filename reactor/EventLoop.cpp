...
EventLoop::EventLoop(bool ismainloop,int timetvl,int timeout)
         :ep_(new Epoll),ismainloop_(ismainloop),wakeupfd_(eventfd(0,EFD_NONBLOCK)),wakeupchannel_(new Channel(this,wakeupfd_))
         ,timerfd_(createtimerfd(timeout)),timerchannel_(new Channel(this,timerfd_)),timetvl_(timetvl),timeout_(timeout),stop_(false)                 
{
    wakeupchannel_->setreadcallback(std::bind(&EventLoop::handwakeup,this));
...
void EventLoop::run()                      
{
    threadid_ = syscall(SYS_gettid);//获取事件循环所在线程id
...
//事件循环线程被唤醒后执行的函数
void EventLoop::handwakeup()
{

    uint64_t val;
    read(wakeupfd_,&val,sizeof(val));  // 从eventfd中读取出数据，如果不读取，eventfd的读事件会一直触发（水平触发）。
...
void EventLoop::handletimer()
{
    ...
    {
        time_t now = time(0);
        std::vector<int> expired_fds; // 暂存超时的 fd

        {
            ...
            for (auto it = conns_.begin(); it != conns_.end(); ) 
            {
                if (it->second->timeout(now, timeout_)) 
                {
                    ...
                    it = conns_.erase(it); // 从 EventLoop 的 conns_ 中删除
                } 
                else 
                {
                    ++it;
        ...
        // 回调 TcpServer::removeconn() 删除 TcpServer 中的连接
        for (int fd : expired_fds) {
            timercallback_(fd);// 从 TcpServer 的 conns_ 中删除
        }
    }
}

//把Connection对象保存在map容器中
